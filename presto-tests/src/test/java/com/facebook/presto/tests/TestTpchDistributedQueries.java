        ...
        extends AbstractTestQueries
{
    ...
    public void testTooLongQuery()
    {
        ...
        @Language("SQL") String longQuery = new StringBuilder().append("SELECT x").append(Strings.repeat(",x", 500_000)).append(" FROM (VALUES 1,2,3,4,5) t(x)").toString();
    ...
    {
        @Language("SQL") String query = new StringBuilder().append("WITH\n").append("  t1 AS (SELECT nationkey AS x FROM nation where name='UNITED STATES'),\n").append("  t2 AS (SELECT a.x+b.x+c.x+d.x AS x FROM t1 a, t1 b, t1 c, t1 d),\n").append("  t3 AS (SELECT a.x+b.x+c.x+d.x AS x FROM t2 a, t2 b, t2 c, t2 d),\n").append("  t4 AS (SELECT a.x+b.x+c.x+d.x AS x FROM t3 a, t3 b, t3 c, t3 d),\n").append("  t5 AS (SELECT a.x+b.x+c.x+d.x AS x FROM t4 a, t4 b, t4 c, t4 d)\n").append("SELECT x FROM t5\n").toString();
