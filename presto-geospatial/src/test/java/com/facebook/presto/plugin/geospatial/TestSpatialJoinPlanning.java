        ...
        extends BasePlanTest
{
    ...
    {
        super(TestSpatialJoinPlanning::createQueryRunner);
    }
    ...
    {
        // broadcast
        assertPlan(new StringBuilder().append("SELECT b.name, a.name ").append("FROM ").append(POINTS_SQL).append(", ").append(POLYGONS_SQL).append(" ").append("WHERE ST_Contains(ST_GeometryFromText(wkt), ST_Point(lng, lat))")
				.toString(),
                anyTree(
        ...
        assertPlan(new StringBuilder().append("SELECT * ").append("FROM (SELECT length(name), * FROM ").append(POINTS_SQL).append("), (SELECT length(name), * FROM ").append(POLYGONS_SQL).append(") ").append("WHERE ST_Contains(ST_GeometryFromText(wkt), ST_Point(lng, lat))")
				.toString(),
                anyTree(
                        spatialJoin("st_contains(st_geometryfromtext, st_point)",
        ...
        // distributed
        assertDistributedPlan(new StringBuilder().append("SELECT b.name, a.name ").append("FROM ").append(POINTS_SQL).append(", ").append(POLYGONS_SQL).append(" ").append("WHERE ST_Contains(ST_GeometryFromText(wkt), ST_Point(lng, lat))")
				.toString(),
    ...
    {
        // broadcast
        assertPlan(new StringBuilder().append("SELECT b.name, a.name ").append("FROM ").append(POINTS_SQL).append(", ").append(POLYGONS_SQL).append(" ").append("WHERE ST_Within(ST_GeometryFromText(wkt), ST_Point(lng, lat))")
				.toString(),
                anyTree(
        ...
        // Verify that projections generated by the ExtractSpatialJoins rule
        // get merged with other projections under the join
        assertPlan(new StringBuilder().append("SELECT * ").append("FROM (SELECT length(name), * FROM ").append(POINTS_SQL).append("), (SELECT length(name), * FROM ").append(POLYGONS_SQL).append(") ").append("WHERE ST_Within(ST_GeometryFromText(wkt), ST_Point(lng, lat))")
				.toString(),
                anyTree(
                        spatialJoin("st_within(st_geometryfromtext, st_point)",
                                project(ImmutableMap.of("st_point", expression("ST_Point(lng, lat)"), "length", expression("length(name)")),
        ...
        // distributed
        assertDistributedPlan(new StringBuilder().append("SELECT b.name, a.name ").append("FROM ").append(POINTS_SQL).append(", ").append(POLYGONS_SQL).append(" ").append("WHERE ST_Within(ST_GeometryFromText(wkt), ST_Point(lng, lat))")
				.toString(),
                withSpatialPartitioning("kdb_tree"),
                anyTree(
    ...
    public void testInvalidKdbTree()
    {
        ...
        assertInvalidSpatialPartitioning(
                ...
                new StringBuilder().append("SELECT b.name, a.name ").append("FROM ").append(POINTS_SQL).append(", ").append(POLYGONS_SQL).append(" ")
						.append("WHERE ST_Contains(ST_GeometryFromText(wkt), ST_Point(lng, lat))").toString(),
    ...
    {
        // broadcast
        assertPlan(new StringBuilder().append("SELECT b.name, a.name ").append("FROM (VALUES ('POLYGON ((30 10, 40 40, 20 40, 10 20, 30 10))', 'a')) AS a (wkt, name), (VALUES ('POLYGON ((30 10, 40 40, 20 40, 10 20, 30 10))', 'a')) AS b (wkt, name) ").append("WHERE ").append(relation).append("(ST_GeometryFromText(a.wkt), ST_GeometryFromText(b.wkt))").toString(),
                anyTree(
    ...
    {
        // distributed
        assertDistributedPlan(new StringBuilder().append("SELECT b.name, a.name ").append("FROM (VALUES ('POLYGON ((30 10, 40 40, 20 40, 10 20, 30 10))', 'a')) AS a (wkt, name), (VALUES ('POLYGON ((30 10, 40 40, 20 40, 10 20, 30 10))', 'a')) AS b (wkt, name) ").append("WHERE ").append(relation).append("(ST_GeometryFromText(a.wkt), ST_GeometryFromText(b.wkt))").toString(),
    ...
    {
        // broadcast
        assertPlan(new StringBuilder().append("SELECT b.name, a.name ").append("FROM (VALUES (2.1, 2.1, 'x')) AS a (lng, lat, name), (VALUES (2.1, 2.1, 'x')) AS b (lng, lat, name) ").append("WHERE ST_Distance(ST_Point(a.lng, a.lat), ST_Point(b.lng, b.lat)) <= 3.1").toString(),
                anyTree(
        ...
        assertPlan(new StringBuilder().append("SELECT b.name, a.name ").append("FROM (VALUES (2.1, 2.1, 'x')) AS a (lng, lat, name), (VALUES (2.1, 2.1, 'x')) AS b (lng, lat, name) ").append("WHERE ST_Distance(ST_Point(a.lng, a.lat), ST_Point(b.lng, b.lat)) <= 300 / (111321 * cos(radians(b.lat)))").toString(),
                anyTree(
                        spatialJoin("st_distance(st_point_a, st_point_b) <= radius",
                                project(ImmutableMap.of("st_point_a", expression("ST_Point(cast(a_lng as double), cast(a_lat as double))")), anyTree(values(ImmutableMap.of("a_lng", 0, "a_lat", 1)))),
        ...
        // distributed
        assertDistributedPlan(new StringBuilder().append("SELECT b.name, a.name ").append("FROM (VALUES (2.1, 2.1, 'x')) AS a (lng, lat, name), (VALUES (2.1, 2.1, 'x')) AS b (lng, lat, name) ").append("WHERE ST_Distance(ST_Point(a.lng, a.lat), ST_Point(b.lng, b.lat)) <= 3.1").toString(),
    ...
    {
        assertPlan(new StringBuilder().append("SELECT b.name, a.name ").append("FROM ").append(POINTS_SQL).append(", ").append(POLYGONS_SQL).append(" ").append("WHERE NOT ST_Contains(ST_GeometryFromText(wkt), ST_Point(lng, lat))")
				.toString(),
                anyTree(
    ...
    {
        assertPlan(new StringBuilder().append("SELECT b.name, a.name ").append("FROM (VALUES ('POLYGON ((30 10, 40 40, 20 40, 10 20, 30 10))', 'a')) AS a (wkt, name), (VALUES ('POLYGON ((30 10, 40 40, 20 40, 10 20, 30 10))', 'a')) AS b (wkt, name) ").append("WHERE NOT ST_Intersects(ST_GeometryFromText(a.wkt), ST_GeometryFromText(b.wkt))").toString(),
                anyTree(
    ...
    {
        assertPlan(new StringBuilder().append("SELECT b.name, a.name ").append("FROM ").append(POINTS_SQL).append(", ").append(POLYGONS_SQL).append(" ").append("WHERE a.name = b.name AND ST_Contains(ST_GeometryFromText(wkt), ST_Point(lng, lat))")
				.toString(),
                anyTree(
    ...
    {
        assertPlan(new StringBuilder().append("SELECT b.name, a.name ").append("FROM (VALUES ('POLYGON ((30 10, 40 40, 20 40, 10 20, 30 10))', 'a')) AS a (wkt, name), (VALUES ('POLYGON ((30 10, 40 40, 20 40, 10 20, 30 10))', 'a')) AS b (wkt, name) ").append("WHERE a.name = b.name AND ST_Intersects(ST_GeometryFromText(a.wkt), ST_GeometryFromText(b.wkt))").toString(),
                anyTree(
                        join(JoinNode.Type.INNER, ImmutableList.of(equiJoinClause("name_a", "name_b")),
    ...
    {
        assertPlan(new StringBuilder().append("SELECT b.name, a.name ").append("FROM ").append(POINTS_SQL).append(" LEFT JOIN ").append(POLYGONS_SQL).append(" ").append("ON ST_Contains(ST_GeometryFromText(wkt), ST_Point(lng, lat))")
				.toString(),
                anyTree(
        ...
        assertPlan(new StringBuilder().append("SELECT b.name, a.name ").append("FROM ").append(POINTS_SQL).append(" LEFT JOIN ").append(POLYGONS_SQL).append(" ").append("ON ST_Contains(ST_GeometryFromText(wkt), ST_Point(lng, lat)) AND a.name <> b.name")
				.toString(),
                anyTree(
        ...
        assertPlan(new StringBuilder().append("SELECT b.name, a.name ").append("FROM ").append(POINTS_SQL).append(" LEFT JOIN ").append(POLYGONS_SQL).append(" ").append("ON ST_Contains(ST_GeometryFromText(wkt), ST_Point(lng, lat)) AND rand() < 0.5")
				.toString(),
                anyTree(
        ...
        // filter over join
        assertPlan(new StringBuilder().append("SELECT b.name, a.name ").append("FROM ").append(POINTS_SQL).append(" LEFT JOIN ").append(POLYGONS_SQL).append(" ").append("   ON ST_Contains(ST_GeometryFromText(wkt), ST_Point(lng, lat)) ")
				.append("WHERE concat(a.name, b.name) is null").toString(),
                anyTree(
    ...
    public void testDistributedSpatialJoinOverUnion()
    {
        ...
        assertDistributedPlan(new StringBuilder().append("SELECT a.name, b.name ").append("FROM (SELECT name FROM tpch.tiny.region UNION ALL SELECT name FROM tpch.tiny.nation) a, tpch.tiny.customer b ").append("WHERE ST_Contains(ST_GeometryFromText(a.name), ST_GeometryFromText(b.name))").toString(),
                withSpatialPartitioning("kdb_tree"),
                anyTree(
        ...
        assertDistributedPlan(new StringBuilder().append("SELECT a.name, b.name ").append("FROM tpch.tiny.customer a, (SELECT name FROM tpch.tiny.region UNION ALL SELECT name FROM tpch.tiny.nation) b ").append("WHERE ST_Contains(ST_GeometryFromText(a.name), ST_GeometryFromText(b.name))").toString(),
                withSpatialPartitioning("kdb_tree"),
                anyTree(
